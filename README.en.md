...
Developers can also clone the repository and run `go install ./cmd/openpe` inside the checkout. Such builds report a development version generated by the Go toolchain from the commit (like `v0.0.0-20260826xxxxxx-commit`), not a release version, so release and development installs stay distinguishable.
...
The command asks the Go module download service (honouring your configured `GOPROXY` mirror) for the latest release, reinstalls both `openpe` and `openpe-server`, and reports the installed version; `openpe update --check` only reports, without installing. When a newer release exists, `pe` feedback also appends one upgrade hint — the check runs in the background at most once per 24 hours and never slows the enhancement itself; set `OPENPE_UPDATE_NOTICE=false` to disable both the hint and the background check. Per-release changes are listed in [CHANGELOG.md](CHANGELOG.md) (Chinese).
...
Requirements you repeat in every prompt (self-check lists, writing rules, commit conventions) can live as plain text files under `~/.config/openpe/specs/<name>.md` — openPE calls these "user specs", and the file name is the reference name. Add `+<name>` after the trigger (several can be chained) and openPE appends the file content verbatim to the end of the enhanced result, delivered together via the clipboard:
...
When a referenced spec is missing, empty, or over the size limit, openPE blocks the enhancement and states the reason instead of silently dropping the constraint you asked for. See [CLIENTS.md](CLIENTS.md#用户规范点名pe) and [CONFIG.md](CONFIG.md#用户规范pe-点名加载) (Chinese).
...
| [CHANGELOG.md](CHANGELOG.md) | User-visible changes and upgrade steps per release (Chinese) |
