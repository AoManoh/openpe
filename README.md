...
| `OPENPE_HISTORY_RATIO`                               | `full`                     | 上下文轮数梯度：控制携带多少会话历史，就近保留最近轮，统一作用于 `flatten`/`hybrid`/`structured`，末轮任务永不裁。`full`（默认，100%＝与实现前逐字节等价、零行为变化）/ `high`（75%）/ `med`（50%）/ `low`（25%）。历史越多指代越强，但过多会稀释模型注意力，需梯度取舍；真正默认档待跨模型 A/B 确定。未设/未知 → `full`，现有配置不受影响 |
