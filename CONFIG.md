...
在对话框输入 `pe+<名字> <任务>`（多份用 `pe+名字1+名字2`）时，openPE 读取规范目录下的 `<名字>.md`，把文件内容逐字追加到增强结果后段（格式为 `[用户规范：<名字>]` 块）。规范不发送给增强模型，只透传给下游 agent，因此不增加增强调用的 token 消耗，也不改变增强质量。

点名的规范找不到、内容为空、超过上限、名字非法或读取失败时，本次增强被阻断并显示具体原因；openPE 不会静默跳过或截断规范。反馈中会列出本次应用的规范名。

规范文档要求：UTF-8 纯文本 `.md`；文件名（不含扩展名）即点名用的名字，支持中文；内容会随增强结果进入剪贴板与下游对话，不要放密钥。

### `OPENPE_SPECS_DIR`

**控制什么**：规范文档所在目录。

**默认值**：`~/.config/openpe/specs`。

```dotenv
OPENPE_SPECS_DIR=/absolute/path/to/specs
```

**修改后的影响**：`pe+<名字>` 与 `openpe enhance --spec <名字>` 都改为在该目录解析；目录中现有规范名会出现在"未找到"错误的提示里。
